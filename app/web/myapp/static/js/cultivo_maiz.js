    ...
    if (pk) {
        try {
            ...
            if (data.status === "success") {
                ...
                console.error("Error al editar el formulario: ", data.errors);
            }
        } catch (error) {
...
const editarCultivo = async (pk) => {
    ...
    try {
        ...
        AccordionSystem.reinitAfterContentLoad(modalForm);
...
const listarCultivoTablaPOST = async (page = 1) => {
    ...
    try {
        ...
        const agregarBtn = document.getElementById('agregar-btn-table');
        if (agregarBtn && isAdmin) {
        ...
        if (cultivos_data_json.length > 0) {
            ...
            headerRow.innerHTML = `
            <tr>
            ${!isAdmin ? '<th colspan="2">Opciones</th>' : ''}
            ...
            cultivos_data_json.forEach(item => {
                ...
                filasHTML += `
                <tr>
                ${!isAdmin ? `<td><button type="button" class="btn-edit" onclick="editarCultivo(${item.pk})">Editar</button></td>
        ...
        construirPaginacion(data.page, data.num_pages);
    } catch (error) {
        console.error(error);
    }
};

const construirPaginacion = (currentPage, totalPages) => {
